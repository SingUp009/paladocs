...
- `render` — 実装済み（型・diff・合成・letterbox・ビューポート連動スケール `scale_for`(pixels-per-point)・セル層 `CellGrid`/半ブロック量子化/罫線描画 `draw_box`・`draw_hline`・`draw_vline`/端末既定色番兵 `DEFAULT`(a=0→SGR `39`/`49`)・テスト緑）
